...
async def main(show_details=False):
    username = "CHECKWATT_USERNAME"
    password = "CHECKWATT_PASSWORD"
    ...
    async with CheckwattManager(username, password) as check_watt_instance:
        try:
            ...
            if await check_watt_instance.login():
                ...
                print("\n")
                print("{:<24}  {:>6}  {:>0}".format("Daily average:", int(check_watt_instance.month_revenue[2]), "kr"))
                print("{:<24}  {:>6}  {:>0}".format("Month estimate:", int(check_watt_instance.month_revenue[3]), "kr")                )
                print("{:<24}  {:>6}  {:>0}".format("Month compensation:", int(check_watt_instance.month_revenue[0]), "kr"))
                print("{:<24}  {:>6}  {:>0}".format("Month fee:", int(check_watt_instance.month_revenue[1]), "kr"))
                print("{:<24}  {:>6}  {:>0}".format("Month revenue:", int(check_watt_instance.month_revenue[0] - check_watt_instance.month_revenue[1]), "kr"))
